	...
	<div class="yotta-main">
		<div class="yotta-main-left" style="background: none">
			<div class="cate-article-list">
				${pageInfo.list?has_content?then("","未找到相关资源")}
			...
			<ul class="pager pager-pills pager-loose">
                <#if pageInfo.hasPreviousPage>
					<li class="previous"><a href="/search/10/${pageInfo.pageNum -1}/${keyword}">«</a></li>
                </#if>

                <#list pageSum as item>
                    <#if item=pageInfo.pageNum>
						<li class="active"><a href="/search/10/${item}/${keyword}">${item}</a></li>
                    <#else>
						<li><a href="/search/10/${item}/${keyword}">${item}</a></li>
                    </#if>
                </#list>
                <#if pageInfo.hasNextPage>
					<li class="next"><a href="/search/10/${pageInfo.pageNum + 1}/${keyword}">»</a></li>
                </#if>
			</ul>
		</div>
        <#include "sider.ftl">
