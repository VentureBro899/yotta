...
export default {
  ...
  mounted() {
    // 获取数据
    this.$request.get(`/article/${this.$route.params.id}`).then(res => {
      this.form = res.data
      this.fileList.push({ name: 'cover', url: this.$store.getters.imgdomain + this.form.coverpicture })
    })

    // 获取分类名称列表
    this.$request.get('/cate/list/1000/1').then(res => {
  ...
  methods: {
    ...
    imgAdd(pos, file) {
      ...
      observable.subscribe({
        ...
        complete(res) {
          that.$refs.md.$img2Url(pos, 'http://rhxlcvfez.hn-bkt.clouddn.com/' + res.key)
        }
      })
    },
    onSubmit() {
