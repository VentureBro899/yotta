    ...
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <el-form-item label="幻灯片">
        <el-upload class="upload-demo" drag name="file" :action="uploadurl" :show-file-list="false" :data="postData" :before-upload="beforeUploadFile"
          ...
          <img v-if="form.picture" :src="imgdomain + form.picture" fit="fill" style="height: 100%;width: 100%;">
          <div v-else>
...
export default {
  ...
  mounted() {
    // 获取数据
    this.$request.get(`/slider/${this.$route.params.id}`).then(res => {
      this.form = res.data
    })
    // 获取上传token
  ...
  methods: {
    ...
    onSubmit() {
      this.$request.put('/slider', this.form).then(res => {
        this.$message({
          message: res.msg,
          type: 'success'
        })
